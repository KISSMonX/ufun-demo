...
uint32_t CIDReg[4];    // cid register data
uint32_t CSDReg[4];    // csd register data
word RCA;           // 卡相对地址
uint8_t CardType;      // SD卡类型
SD_CardInfo SDInfo; // SD卡所有信息

uint8_t SDIO_DATA_BUFFER[512];
uint8_t DeviceMode;
SD_ERROR TransferError=SD_OK;
uint8_t StopCondition=0;			//单块读,不需要发送停止传输指令
uint8_t TransferEnd=0;
uint8_t TotalNumberOfuint8_ts = 0;


/**********************************************
  SDIO_CK=SDIOCLK / [CLKDIV + 2]
 ***********************************************/
inline void SDIO_CK_Set(uint8_t div)
{
...
inline void SDIO_PWRCTRL(uint8_t State)
{
	SDIO_POWER=State;
}
 ...
 ***********************************************/
void SDIO_SendCMD(uint8_t cmdindex,uint32_t arg,uint8_t waitResp)
{
	SDIO_ARG=arg;
	SDIO_CMD &= 0xfffff800;
	SDIO_CMD |= cmdindex;
 ...
 ***********************************************/
void SDIO_DataConfig(uint32_t datatimeout,uint32_t datalen,uint8_t blocksize,uint8_t datadir)
{
 ...
 **********************************************/
void SD_DMA2_Config(uint32_t*mbuf,uint32_t bufsize,uint8_t dir)
{
	...
	DMA2_CNDTR4=bufsize>>2;//DMA2,传输数据量 uint8_t->uint32_t
	DMA2_CPAR4=(uint32_t)&SDIO_FIFO;//DMA2 外设地址
	DMA2_CMAR4=(uint32_t)mbuf; 		//DMA2,存储器地址
...
/**********************************************
  R1响应
 ***********************************************/
SD_ERROR CMDResponse1(uint8_t CMD)
{
	uint32_t value;
	while(1)    // 等待R1响应
	{
...
/**********************************************
  R6响应(RCA)
 ***********************************************/
SD_ERROR CMDResponse6(uint8_t CMD,word *prca)
{
	uint32_t value;
	uint32_t resp;
	while(1)    // 等待R1响应
...
SD_ERROR SD_InitializeCard()
{
			...
			(SDIO_HIGH_CAPACITY_SD_CARD == CardType))//判断卡类型
	{
		...
		SDIO_SendCMD(SD_CMD_SET_REL_ADDR,(uint32_t)(rca<<16),1);//发送CMD3,短响应
	...
	if (SDIO_SECURE_DIGITAL_IO_CARD != CardType){
		...
		SDIO_SendCMD(SD_CMD_SEND_CSD,(uint32_t)(rca<<16),3);
		status=CMDResponse2(); 					//等待R2响应
		if(status!=SD_OK)return status;   	//响应错误
		CSDReg[0]=SDIO_RESP1;
		CSDReg[1]=SDIO_RESP2;
...
void SD_GetCardInfo(SD_CardInfo *cardinfo)
{
	...
	tmp=(uint8_t)((CSDReg[0]&0xFF000000)>>24);
			...
			(SDIO_MULTIMEDIA_CARD==CardType))       //标准1.1/2.0卡/MMC卡
	{
		...
		tmp=(uint8_t)(CSDReg[1]&0x000000FF);      //第7个字节
		cardinfo->SD_csd.DeviceSize|=(tmp)<<2;
		tmp=(uint8_t)((CSDReg[2]&0xFF000000)>>24);    //第8个字节
		cardinfo->SD_csd.DeviceSize|=(tmp&0xC0)>>6;
		cardinfo->SD_csd.MaxRdCurrentVDDMin=(tmp&0x38)>>3;
		cardinfo->SD_csd.MaxRdCurrentVDDMax=(tmp&0x07);
		tmp=(uint8_t)((CSDReg[2]&0x00FF0000)>>16);    //第9个字节
		cardinfo->SD_csd.MaxWrCurrentVDDMin=(tmp&0xE0)>>5;
		cardinfo->SD_csd.MaxWrCurrentVDDMax=(tmp&0x1C)>>2;
		cardinfo->SD_csd.DeviceSizeMul=(tmp&0x03)<<1;//C_SIZE_MULT
		tmp=(uint8_t)((CSDReg[2]&0x0000FF00)>>8);     //第10个字节
	...
	tmp=(uint8_t)(CSDReg[2]&0x000000FF);      //第11个字节
	cardinfo->SD_csd.EraseGrMul|=(tmp&0x80)>>7;
	cardinfo->SD_csd.WrProtectGrSize=(tmp&0x7F);
	tmp=(uint8_t)((CSDReg[3]&0xFF000000)>>24);    //第12个字节
	cardinfo->SD_csd.WrProtectGrEnable=(tmp&0x80)>>7;
	cardinfo->SD_csd.ManDeflECC=(tmp&0x60)>>5;
	cardinfo->SD_csd.WrSpeedFact=(tmp&0x1C)>>2;
	cardinfo->SD_csd.MaxWrBlockLen=(tmp&0x03)<<2;
	tmp=(uint8_t)((CSDReg[3]&0x00FF0000)>>16);    //第13个字节
	cardinfo->SD_csd.MaxWrBlockLen|=(tmp&0xC0)>>6;
	cardinfo->SD_csd.WriteBlockPaPartial=(tmp&0x20)>>5;
	cardinfo->SD_csd.Reserved3=0;
	cardinfo->SD_csd.ContentProtectAppli=(tmp&0x01);
	tmp=(uint8_t)((CSDReg[3]&0x0000FF00)>>8);   //第14个字节
	...
	tmp=(uint8_t)(CSDReg[3]&0x000000FF);      //第15个字节
	cardinfo->SD_csd.CSD_CRC=(tmp&0xFE)>>1;
	cardinfo->SD_csd.Reserved4=1;

	tmp=(uint8_t)((CIDReg[0]&0xFF000000)>>24);    //第0个字节
	cardinfo->SD_cid.MID=tmp;
	tmp=(uint8_t)((CIDReg[0]&0x00FF0000)>>16);    //第1个字节
	cardinfo->SD_cid.OID=tmp<<8;
	tmp=(uint8_t)((CIDReg[0]&0x000000FF00)>>8);   //第2个字节
	cardinfo->SD_cid.OID|=tmp;
	tmp=(uint8_t)(CIDReg[0]&0x000000FF);      //第3个字节
	cardinfo->SD_cid.PNM1=tmp<<24;
	tmp=(uint8_t)((CIDReg[1]&0xFF000000)>>24);    //第4个字节
	cardinfo->SD_cid.PNM1|=tmp<<16;
	tmp=(uint8_t)((CIDReg[1]&0x00FF0000)>>16);      //第5个字节
	cardinfo->SD_cid.PNM1|=tmp<<8;
	tmp=(uint8_t)((CIDReg[1]&0x0000FF00)>>8);   //第6个字节
	cardinfo->SD_cid.PNM1|=tmp;
	tmp=(uint8_t)(CIDReg[1]&0x000000FF);        //第7个字节
	cardinfo->SD_cid.PNM2=tmp;
	tmp=(uint8_t)((CIDReg[2]&0xFF000000)>>24);    //第8个字节
	cardinfo->SD_cid.PRV=tmp;
	tmp=(uint8_t)((CIDReg[2]&0x00FF0000)>>16);    //第9个字节
	cardinfo->SD_cid.PSN=tmp<<24;
	tmp=(uint8_t)((CIDReg[2]&0x0000FF00)>>8);     //第10个字节
	cardinfo->SD_cid.PSN|=tmp<<16;
	tmp=(uint8_t)(CIDReg[2]&0x000000FF);        //第11个字节
	cardinfo->SD_cid.PSN|=tmp<<8;
	tmp=(uint8_t)((CIDReg[3]&0xFF000000)>>24);    //第12个字节
	cardinfo->SD_cid.PSN|=tmp;
	tmp=(uint8_t)((CIDReg[3]&0x00FF0000)>>16);    //第13个字节
	cardinfo->SD_cid.Reserved1|=(tmp&0xF0)>>4;
	cardinfo->SD_cid.MDT=(tmp&0x0F)<<8;
	tmp=(uint8_t)((CIDReg[3]&0x0000FF00)>>8);   //第14个字节
	cardinfo->SD_cid.MDT|=tmp;
	tmp=(uint8_t)(CIDReg[3]&0x000000FF);      //第15个字节
...
pscr:数据缓存区(存储SCR内容)
 **********************************************/
SD_ERROR ReadSCR(word rca,uint32_t *pscr)
{
	uint32_t index = 0;
	SD_ERROR status = SD_OK;
	uint32_t tempscr[2]={0,0};

	//Send CMD16: Set Block Size To 8 uint8_ts
	SDIO_SendCMD(SD_CMD_SET_BLOCKLEN,8,1);//短响应,设置Block Size为8字节
	status=CMDResponse1(SD_CMD_SET_BLOCKLEN);
	if(status!=SD_OK)return status;

	//Send CMD55 APP_CMD with argument as card's RCA
	SDIO_SendCMD(SD_CMD_APP_CMD,(uint32_t)rca<<16,1);
	status=CMDResponse1(SD_CMD_APP_CMD);
	if(status!=SD_OK)return status;
...
enable:0,不使能;1,使能;
 **********************************************/
SD_ERROR SDCardEnWideBus(uint8_t enable)
{
	SD_ERROR status = SD_OK;
	uint32_t scr[2]={0,0};
	uint8_t arg=0X00;      // 1bit
	if(enable)arg=0X02; // 4bit

	//-------CMD13:send card status with RCA-------//
	SDIO_SendCMD(SD_CMD_SEND_STATUS,(uint32_t)RCA<<16,1);
 ...
 **********************************************/
SD_ERROR SD_SelectCard(uint32_t rca)
{
	SD_ERROR status=SD_OK;
	// send CMD7
...
/**********************************************
  得到NumberOfuint8_ts以2为底的指数.
NumberOfuint8_ts:字节数.
返回值:以2为底的指数值
 **********************************************/
uint8_t convert_from_uint8_ts_to_power_of_two(word NumberOfuint8_ts)
{
	uint8_t count=0;
	while(NumberOfuint8_ts!=1)
	{
		NumberOfuint8_ts>>=1;
		count++;
	}
 ...
 **********************************************/
SD_ERROR GetSDCardMode(uint8_t * curstatus)
{
	uint32_t resp = 0;
	SD_ERROR status = SD_OK;
	SDIO_SendCMD(SD_CMD_SEND_STATUS,(uint32_t)RCA<<16,1);//发送CMD13
	status=CMDResponse1(SD_CMD_SEND_STATUS);
	if(status!=SD_OK)return status;
	resp=SDIO_RESP1;
	*curstatus=(uint8_t)((resp>>9)&0x0000000F);
 ...
 *dataBuf_指向存储读取数据的存储区的指针
 blockSize_数据块大小,SD卡固定为512uint8_t
 **********************************************/
SD_ERROR SD_ReadSingleBlock(uint8_t *dataBuf,uint32_t addr,word blockSize)
{
	SD_ERROR status=SD_OK;
	uint32_t count=0,*tempBuff=(uint32_t *)dataBuf;
	uint8_t power=0;
	uint32_t timeout;
	...
	{
		power=convert_from_uint8_ts_to_power_of_two(blockSize);
	...
	TotalNumberOfuint8_ts = blockSize;
	StopCondition = 0;
	//DestBuffer = dataBuf;
	//--------------发送CMD17-------------------//
	//从addr地址出读取数据,短响应
	...
	else if(DeviceMode==SD_DMA_MODE)
	{
		...
		SD_DMA2_Config((uint32_t *)dataBuf,blockSize,0);
...
//返回值:错误状态
 **********************************************/
SD_ERROR SD_ReadMultiBlocks(uint8_t *buf,uint32_t addr,word blksize,uint32_t nblks)
{
	SD_ERROR status=SD_OK;
	uint8_t power;
	...
	if((blksize>0)&&(blksize<=2048)&&((blksize&(blksize-1))==0)){
		power=convert_from_uint8_ts_to_power_of_two(blksize);
	...
	if(nblks>1){  //多块读
		...
		else if(DeviceMode==SD_DMA_MODE)
		{
			...
			SD_DMA2_Config((uint32_t*)buf,nblks*blksize,0);
			timeout=SDIO_DATATIMEOUT;
...
返回值:错误状态
 **********************************************/
SD_ERROR SD_WriteBlock(uint8_t *buf,uint32_t addr,word blksize)
{
	SD_ERROR status = SD_OK;
	uint8_t  power=0,cardstate=0;
	uint32_t timeout=0,uint8_tstransferred=0;
	...
	SDIO_SendCMD(SD_CMD_SEND_STATUS,(uint32_t)RCA<<16,1);//发送CMD13,查询卡的状态,短响应
	...
	if(DeviceMode == SD_POLLING_MODE){
						...
						SDIO_STA_STBITERR|SDIO_STA_DBCKEND)))//数据块发送成功/下溢/CRC/超时/起始位错误
		{
			...
			{
				if((tlen-uint8_tstransferred)<SD_HALFFIFOuint8_tS)//不够32字节了
				{
					restwords=((tlen-uint8_tstransferred)%4==0)?((tlen-uint8_tstransferred)/4):((tlen-uint8_tstransferred)/4+1);
					for(count=0;count<restwords;count++,tempbuff++,uint8_tstransferred+=4)
					{
						SDIO_FIFO=*tempbuff;
					}
				}else{
	...
	else if(DeviceMode==SD_DMA_MODE)
	{
		...
		SDIO_MASK=SDIO_MASK_DCRCFAILIE|SDIO_MASK_DTIMEOUTIE|SDIO_MASK_TXUNDERRIE|
			SDIO_MASK_DATAENDIE|SDIO_MASK_STBITERRIE;
		SD_DMA2_Config((uint32_t*)buf,blksize,1);//SDIO DMA配置
		SDIO_DCTRL |= SDIO_DCTRL_DMAEN;//SDIO DMA使能
		timeout=SDIO_DATATIMEOUT;
		while(((DMA2_ISR&BIT13)== 0 )&&timeout)timeout--;//等待传输完成
		if(timeout==0){
 ...
 **********************************************/
SD_ERROR SD_WriteMultiBlocks(uint8_t *buf,uint32_t addr,word blksize,uint32_t nblks)
{
	SD_ERROR status = SD_OK;
	uint8_t  power = 0, cardstate = 0;
	uint32_t timeout=0,uint8_tstransferred=0;
	...
	if(nblks>1){
							...
							SDIO_STA_STBITERR|SDIO_STA_DATAEND)))//下溢/CRC/数据结束/超时/起始位错误
			{
				if(SDIO_STA&SDIO_STA_TXFIFOHE){//发送区半空,表示至少存了8字(32字节)
					if((tlen-uint8_tstransferred)<SD_HALFFIFOuint8_tS)//不够32字节了
					{
						...
						uint8_tstransferred+=SD_HALFFIFOuint8_tS;
					}
				}
			}
			if(SDIO_STA&SDIO_STA_DTIMEOUT){   //数据超时错误
		...
		else if(DeviceMode==SD_DMA_MODE)
		{
			...
			SDIO_MASK=SDIO_MASK_DCRCFAILIE|SDIO_MASK_DTIMEOUTIE|SDIO_MASK_TXUNDERRIE|
				SDIO_MASK_DATAENDIE|SDIO_MASK_STBITERRIE;
			SD_DMA2_Config((uint32_t*)buf,nblks*blksize,1);//SDIO DMA配置
			SDIO_DCTRL |= SDIO_DCTRL_DMAEN;//SDIO DMA使能
			timeout=SDIO_DATATIMEOUT;
			while(((DMA2_ISR&BIT13)== 0 )&&timeout)timeout--;//等待传输完成
			if(timeout==0){//超时
